...
Before you start, you need to create an s3 bucket unique for you named like `terraform-di-[student-alias]`. You could do it using Terraform or manually (at least for now). We'll be using this bucket as the place to store and manage your remote state as we move forward in this course. We'll be working primarily with realistic, remote state scenarios for the remainder of our exercises.
...
$ terraform providers mirror ./custom-plugin-dir/
```
...
```
$ terraform init -plugin-dir=./custom-plugin-dir -backend-config=./backend.tfvars -backend-config=bucket=terraform-di-[student-alias]
```

And we'll have an updated look at our `.terraform` directory and its contents.

Notice that the folder structure looks similar as before but it's pointing to a different plugin path where it's "downloading" all the binaries. The `.terraform/plugin_path` points terraform to a local location where it can find the provider plugin to use:
