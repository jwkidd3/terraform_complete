...
We want this exercise to be a refresher on some key topics, and adding just a few new concepts or twists as we jump back into a path toward intermediate/advanced knowledge.
...
- security_group in .terraform/modules/security_group/terraform-aws-security-group-4.0.0
...
`Downloading plugin for provider "aws" (hashicorp/aws) 2.70.0...` from the logs indicates that it found the version it needs and is downloading it. This provider, or plugin is also pulled into the `.terraform` directory:
...
Create the infrastructure and confirm that everything is working:

```
terraform apply
```

That's it for this refresher exercise.
